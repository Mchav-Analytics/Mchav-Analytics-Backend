...
def get_team_performance_matrix(
    ...
    """
    pid = resolve_project_id(db, proyecto_id)
    try:
        return calculate_team_performance_matrix(db, pid, sprint_id)
...
def get_my_scorecard(
    ...
    return get_developer_scorecard_data(db, pid, email_or_assignee_id=email)
...
def get_my_daily_focus(
    ...
    pid = resolve_project_id(db, proyecto_id)
    email = current_user.email if current_user else None
    return get_daily_focus_data(db, pid, email_or_assignee_id=email)
...
def get_my_alerts(
    ...
    pid = resolve_project_id(db, proyecto_id)
    email = current_user.email if current_user else None
    return get_developer_alerts_data(db, pid, email_or_assignee_id=email)
...
def get_my_activity_history(
    ...
    pid = resolve_project_id(db, proyecto_id)
    email = current_user.email if current_user else None
    return get_activity_history_data(db, pid, email_or_assignee_id=email)
...
def get_my_issues(
    ...
    pid = resolve_project_id(db, proyecto_id)
    email = current_user.email if current_user else None
    scorecard = get_developer_scorecard_data(db, pid, email_or_assignee_id=email)
    return {
        "proyecto_id": pid,
...
def get_developer_scorecard_by_id(
    ...
    """
    pid = resolve_project_id(db, proyecto_id)
    return get_developer_scorecard_data(db, pid, email_or_assignee_id=assignee_id)
