"""DTOs de integración Jira (JQL y sync)."""

from pydantic import BaseModel, Field


class JqlSearchRequestDTO(BaseModel):
...
class JqlSearchResponseDTO(BaseModel):
    total: int
    start_at: int
    max_results: int
    issues: list[dict]
...
class SyncProjectRequestDTO(BaseModel):
    project_key: str = Field(..., min_length=2, max_length=20)


class SyncProjectResponseDTO(BaseModel):
    project_key: str
    issues_synced: int
    sprints_synced: int
    duration_sec: float
